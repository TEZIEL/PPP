...
namespace PPP.BLUE.VN
{
    public sealed class VNDialogueView : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (inputLocked)
            {
                Debug.Log($"[VN_READY_CHECK] not ready externalBlocked={externalInputBlocked} inputLocked={inputLocked} isTyping={(typer != null && typer.IsTyping)} displayed={lineDisplayed} isWaitingInput={lineCompleted} runnerNull={(runner == null)} runnerSaveAllowed={(runner != null && runner.SaveAllowed)} currentFullTextEmpty={string.IsNullOrEmpty(currentFullText)} dialogueText={(dialogueText != null ? dialogueText.text : "<null>")} nameText={(nameText != null ? nameText.text : "<null>")} saveLoadOpen={(saveLoadWindow != null && saveLoadWindow.gameObject.activeInHierarchy)} policyModal={(policy != null && policy.IsModalOpen)} activeInHierarchy={gameObject.activeInHierarchy}");
